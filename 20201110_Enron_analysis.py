...
# =============================================================================
# SET-UP - Python modules necessary for project
# =============================================================================
#See file "References" in GitHub for source used below

## Modules for creating pickle files and run tester
import pickle
import sys
sys.path.append("../tools/")
from tester import dump_classifier_and_data 
file_path = "../final_project/final_project_dataset.pkl"
from io import StringIO #not used in this query but might be requested for tester ?

data_dict = pickle.load(open(file_path, "rb"), fix_imports=True)
#data_dict = pickle.load(open("../final_project/final_project_dataset.pkl", "r") )#only in python 2.7, replaced by line 37 in python 3

##Modules necessary for graphs : added one after the other, not sure some are not redundant
import matplotlib
#
...
##For better looking display of lists, dataframes and better use of the full width of the screen##
...
# =============================================================================
# IMPORT DATASET, SIZE IT, SEE AVAILABLE FEATURES
# =============================================================================
...
# =============================================================================
##I will not suppress all NAN values because I can see that absence of data for some features mean that they do not exists, not that 
## data was not known : getting NAN for bonus means that this person received no bonus.
## could have been automated with sklearn module 
...
# =============================================================================
#There must be a smarter leaner way to proceed, but I did not find it
...
#feature list1 is the list of all features
...
#feature list0 will be used for final classifier
features_list0 =['part_from_POI','part_to_POI','ratio_bonus_salary','director_fees','from_messages','from_poi_to_this_person','total_payments'
                 ]
...
X_train,y_train=prepa(trainset,features_list1)
X_test,y_test=prepa(testset,features_list1)
...
dot_data = tree.export_graphviz(resultat, out_file=None,feature_names=X_train.columns,class_names=dt_target_names,filled=True,rounded=True,leaves_parallel=(True))
...
# =============================================================================
# Chose best parameters and best features selection
...
# =============================================================================
# prevents lot of lines errors messages, but hides the fact that selectorbest does not work with Chi2
# =============================================================================
warnings.filterwarnings('ignore', 'Solver terminated early.*')
warnings.filterwarnings('ignore',"Estimator fit failed. The score on this train-test")
# =============================================================================
# 
# =============================================================================
...
search_space = [{'scaler':[StandardScaler(),MinMaxScaler(),RobustScaler(),None]},
                ...
                {'classifier':[DecisionTreeClassifier()],
                 ...
                 'classifier__min_samples_split':[2,3,4,5,6,8,10],
...
clf = GridSearchCV(pipe, search_space, cv=3,scoring='f1', verbose=10)
clf = clf.fit(X_train, y_train)
output=pd.DataFrame(clf.cv_results_)

outputsel=output.loc[0:,['param_scaler','param_classifier','param_selector__k','rank_test_score','mean_test_score']]
outputselsort=outputsel.sort_values(by ='rank_test_score')

print(output.columns)

print(tabulate(outputselsort.head(100)
...
# =============================================================================
# IDENTIFY 7 BEST PARAMETERS WITH SVC
# =============================================================================
...
Selector.fit_transform(X_train,y_train)
selection=Selector.get_support()
for a,b in zip(features_list1,selection) :
    if b == True :
        print(a)
...
N, train_score, val_score = learning_curve(clf, X_train, y_train,
                                                 cv=3,scoring='f1', train_sizes=np.linspace(0.1,1,10))
